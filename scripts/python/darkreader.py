...
class Darkreader(object):
    ...
    def get_scriptpath(self, url_pattern):
        ...
        grease_filename = f'_g.dm.{url_pattern_md5.hexdigest()}.js'
    ...
    def toggle(self, url, url_pattern, brightness, contrast, sepia):
        if os.path.isfile(self.get_scriptpath(url_pattern)):
            self.disable(url_pattern)
        else:
            self.enable(url, url_pattern, brightness, contrast, sepia)
...
if __name__ == "__main__":
    ...
    darkreader.toggle(url, url_pattern, brightness, contrast, sepia)
